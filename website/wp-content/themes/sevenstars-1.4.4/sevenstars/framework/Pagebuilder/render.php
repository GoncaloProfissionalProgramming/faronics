    ...
    if ($post_types != '') {
        foreach ($post_types as $key => $value) {
            if (blox_post_type() == $value) {
                ...
                include_once file_require(BLOX_DIR . 'items/items.php');
                $files = array();
                $file_index = 0;
                global $blox_items;
...
/* Simple Line Icons */
$font_pointer = fs_get_contents( get_template_directory()."/assets/plugins/simple-line-icons/icons.txt" );
if( !empty($font_pointer) ){
    $font_pointer_ex = explode("\n", $font_pointer);
    foreach ($font_pointer_ex as $exitem) {
        $font_item = trim($exitem . '');
        $font_awesome[] = $font_item;
    }
}

/* Font Awesome */
$font_pointer = fs_get_contents( get_template_directory()."/assets/plugins/font-awesome/less/icons.less" );
if( !empty($font_pointer) ){
    $font_pointer_ex = explode("\n", $font_pointer);
    foreach ($font_pointer_ex as $exitem) {
        $font_item = trim($exitem . '');
...
function blox_pagebuilder_section() {
    ...
    if ($post_types != '') {
        foreach ($post_types as $key => $value) {
            add_meta_box(
                'blox_contentbuilder', __('Blox Content Builder', 'sevenstars'), 'render_blox_pagebuilder', $value, 'normal', 'high'
            );
        }
    }
}
