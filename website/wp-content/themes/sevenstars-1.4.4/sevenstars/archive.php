			...
			<div class="col-md-12">
				<div class="">
					...
					if (is_author()) {
                        ...
                        if(have_posts()) { 
                            the_post();
                            echo '<h3>'.__("Written by ", 'sevenstars') . get_the_author().'</h3>';
						...
						// Title text
						if(is_tax('portfolio_entries'))
							printf(__('Portfolio Entries : %s', 'sevenstars'), single_cat_title('', false));
	                    elseif (is_category()) {
	                        printf(__('Category : %s', 'sevenstars'), single_cat_title('', false));
	                    } elseif (is_tag()) {
	                        printf(__('Tag Archives: %s', 'sevenstars'), single_tag_title('', false));
	                    } elseif (is_archive()) {
	                        if (is_day()) :
	                            printf(__('Daily Archives: %s', 'sevenstars'), get_the_date());
	                        elseif (is_month()) :
	                            printf(__('Monthly Archives: %s', 'sevenstars'), get_the_date(_x('F Y', 'monthly archives date format', 'sevenstars')));
	                        elseif (is_year()) :
	                            printf(__('Yearly Archives: %s', 'sevenstars'), get_the_date(_x('Y', 'yearly archives date format', 'sevenstars')));
	                        else :
	                            _e('Archives', 'sevenstars');
	                        endif;
	                    }
	                    ?>
	                    </h1>
								...
								<div class="col-md-12">
		                            ...
		                            if (have_posts()) :
		                            	...
		                            	$loop_args = array(
							                            'overlay' => 'none',
							                            'excerpt' => 'excerpt',
							                            'readmore' => __('Read more', 'sevenstars'),
							                            'grid' => $columns,
